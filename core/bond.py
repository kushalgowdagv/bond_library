...
"""
Bond interfaces and base classes with discrete compounding
"""
...
class Bond(ABC):
    ...
    def price(self, valuation_date: datetime, curve: InterestRateCurve) -> float:
        """
        Calculate clean price of the bond using discrete compounding
    ...
    def yield_to_maturity(self, valuation_date: datetime, market_price: float) -> float:
        """
        Calculate yield to maturity given a market price using discrete compounding
        ...
        def price_function(ytm: float) -> float:
            """Calculate bond price for a given yield using discrete compounding"""
            pv = 0.0
            for cf in cash_flows:
                time_to_cf = DateUtils.year_fraction(valuation_date, cf.payment_date)
                # Use discrete compounding with payment frequency
                discount_factor = 1.0 / ((1.0 + ytm / self.payment_frequency) ** (time_to_cf * self.payment_frequency))
        ...
        def price_derivative(ytm: float) -> float:
            ...
            for cf in cash_flows:
                time_to_cf = DateUtils.year_fraction(valuation_date, cf.payment_date)
                periods = time_to_cf * self.payment_frequency
                discount_factor = 1.0 / ((1.0 + ytm / self.payment_frequency) ** periods)
                # Derivative of discrete compounding formula
                dpv -= cf.amount * periods * discount_factor / (1.0 + ytm / self.payment_frequency)
            return dpv
    ...
    def duration(self, valuation_date: datetime, yield_rate: float) -> float:
        """
        Calculate Macaulay duration using discrete compounding
        ...
        for cf in cash_flows:
            time_to_cf = DateUtils.year_fraction(valuation_date, cf.payment_date)
            # Use discrete compounding with payment frequency
            discount_factor = 1.0 / ((1.0 + yield_rate / self.payment_frequency) ** (time_to_cf * self.payment_frequency))
            pv = cf.amount * discount_factor
    ...
    def modified_duration(self, valuation_date: datetime, yield_rate: float) -> float:
        """
        Calculate modified duration using discrete compounding
        ...
        # For discrete compounding: Modified Duration = Macaulay Duration / (1 + yield/frequency)
        return mac_duration / (1.0 + yield_rate / self.payment_frequency)
    
    def convexity(self, valuation_date: datetime, yield_rate: float) -> float:
        """
        Calculate convexity using discrete compounding
        ...
        for cf in cash_flows:
            ...
            periods = time_to_cf * self.payment_frequency
            # Use discrete compounding with payment frequency
            discount_factor = 1.0 / ((1.0 + yield_rate / self.payment_frequency) ** periods)
            pv = cf.amount * discount_factor
            
            # Convexity formula for discrete compounding
            factor = periods * (periods + 1) / ((1.0 + yield_rate / self.payment_frequency) ** 2)
            convexity_weighted_sum += factor * pv
            price_sum += pv
            
        if price_sum == 0:
            return 0.0
            
        return convexity_weighted_sum / (price_sum * (self.payment_frequency ** 2))
    ...
    def _price_from_yield(self, valuation_date: datetime, yield_rate: float) -> float:
        """Helper method to calculate price from yield using discrete compounding"""
        cash_flows = self.get_remaining_cash_flows(valuation_date)
        pv = 0.0
